...
function processData() {
    ...
    if (
        ...
        if (isGoingToRain() == true && stopRaining < 4) {
            console.log(
                `Está chovendo e volverá a chover en ${nextRain} horas`
            );
        }
        showPositiveRaining({
            location: 'Test',
        ...
        if (isGoingToRain() == true) {
            showPositive({
                ...
                textWeather:
                    weatherCodes[
                        parseInt(prediction.current_weather.weathercode)
                    ],
            });
        } else {
            showNegative({
...
async function processLocation(location) {
    ...
    try {
        ...
        const roundedCurrentDate =
            currentHour.toISOString().slice(0, 13) + ':00';
        index = prediction.hourly.time.indexOf(roundedCurrentDate);
